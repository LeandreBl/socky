...
Test(socky, invalid_param_recv)
{
    struct socky client;
    uint8_t buffer[256];

    cr_assert(socky_create(&client, SOCKY_TCP) == 0);
    cr_assert(socky_recv(&client, buffer, sizeof(buffer), 0) == -1);
    cr_assert(errno == ENOTCONN);
    cr_assert(socky_destroy(&client) == 0);
}

Test(socky, recv_on_listener)
{
    struct socky client;
    uint8_t buffer[256];
...
Test(socky, normal_recv)
{
    ...
    cr_assert(socky_recv(&client, buffer, sizeof(buffer), 0) == (ssize_t)strlen(bytes));
