...
Test(socky, connect_from_str_with_netcat)
{
    struct socky client;
    char *cmd = NULL;
    uint16_t port;
    ...
    sleep(2);
    cr_assert(socky_connect_from_string(&client, addr, port) == 0);
...
Test(socky, connect_from_str_with_netcat_udp)
{
    struct socky client;
    char *cmd = NULL;
    uint16_t port;
...
Test(socky, connect_from_str_to_1_1_1_1)
{
    struct socky client;
    uint16_t port = 80;
    ...
    cr_assert(socky_create(&client, SOCKY_TCP) == 0);
    cr_log_warn("Connecting (TCP) to %18s:%5d\n", addr, port);
    cr_assert(socky_connect_from_string(&client, "1.1.1.1", port) == 0);
    cr_assert(client.fd != -1);
    cr_assert(client.proto == SOCKY_TCP);
    cr_assert(client.state == SOCKY_CONNECTED);
    cr_assert(client.type == SOCKY_DUPLEX);
...
Test(socky, socky_connect_from_str_to_1_1_1_1_udp)
{
    struct socky client;
    uint16_t port = 80;
    const char *addr = "1.1.1.1";
