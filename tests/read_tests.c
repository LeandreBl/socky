...
Test(socky, invalid_param)
{
    struct socky client;
    uint8_t buffer[256];

    cr_assert(socky_create(&client, SOCKY_TCP) == 0);
    cr_assert(socky_read(&client, buffer, sizeof(buffer)) == -1);
    cr_assert(errno == ENOTCONN);
    cr_assert(socky_destroy(&client) == 0);
}

Test(socky, read_on_listener)
{
    struct socky client;
    uint8_t buffer[256];
...
Test(socky, normal_read)
{
    ...
    cr_assert(socky_read(&client, buffer, sizeof(buffer)) == (ssize_t)strlen(bytes));
